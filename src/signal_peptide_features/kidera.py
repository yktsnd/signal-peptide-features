"""Kidera-10 amino-acid factor summaries."""
...
def kidera_summary(sequence: str) -> dict[str, float]:
    normalized = normalize_sequence(sequence)
    matrix = np.asarray([KIDERA_FACTORS[residue] for residue in normalized], dtype=np.float64)
    return {
        **{f"kidera_f{index + 1}_mean": float(matrix[:, index].mean()) for index in range(10)},
        **{f"kidera_f{index + 1}_std": float(matrix[:, index].std()) for index in range(10)},
    }
